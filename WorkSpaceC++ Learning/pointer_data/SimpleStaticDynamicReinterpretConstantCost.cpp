...
-What is the difference Simple cast and Re_interpret cast as both does the same in c++,
 Which is better
-Simple cast check for all type casting,which mach it does,that means in case of overide,
  it will check dynamic cast
-When we should use static cast, dynamic cast,reinterpret_cast, take this case (Base *ptrB1=  new Derived)
-as it is successful without casting,so why we are going for static_cast and dynamic cast?
...
-Class conversion
	...
	-Differnce b/w static cast and synamic cast in case of Inheritance upcasting and
	 down casting,Give Example.In one case casting will be succefull but retune pointer
	 will be null,(exploree it)
=====================================================================================================
#endif
...
//class conversion---------------------------------------------------------
		//Upcasting
		Base *ptrB1=  new Derived;//error if not derived publicly
...
=============================================================================================
Both reinterpret_cast and c-style casts are implementation defined and they do almost the same
thing. The differences are :
-------------------------------------------------------
1. reinterpret_cast can not remove constness. For example :
const unsigned int d = 5;
int *g=reinterpret_cast< int* >( &d ); //will issue an error :
error: reinterpret_cast from type 'const unsigned int*' to type 'int*' casts away qualifiers

2. If you use reinterpret_cast, it is easy to find the places where you did it. It is not possible
to do with c-style casts
#endif
#if 0
#endif
